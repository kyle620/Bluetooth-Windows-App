		...
		if (bgapiRXBufferPos == 2) {
			...
			if ((bgapiRXBuffer[0] & 0x80) == 0) {
				...
				if (bgapiRXBuffer[2] == 0) {
					...
					else if (bgapiRXBuffer[3] == 0) { if (ble_evt_system_boot) { ble_evt_system_boot((const struct ble_msg_system_boot_evt_t *)(bgapiRXBuffer + 4)); }  }
					...
					else if (bgapiRXBuffer[3] == 0) { if(ble_evt_hardware_adc_result) ble_evt_dfu_boot((const struct ble_msg_dfu_boot_evt_t *)(bgapiRXBuffer + 4)); }
				}
			}
		}
	}
